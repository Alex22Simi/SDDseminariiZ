...
typedef struct StructuraMasina Masina; //typedef creeaza un alias
...
void adaugaMasinaInVector(Masina** masini, int* nrMasini, Masina masinaNoua) {
	...
	aux = (Masina*)malloc(sizeof(Masina) * ((*nrMasini)+1));
	for (int i = 0; i < (*nrMasini); i++) {
		aux[i] = (*masini)[i];//shallow coppy
	}
	aux[*(nrMasini)] = masinaNoua; //shallow
	free((*masini));
	(*masini) = aux;
	(*nrMasini) = (*nrMasini) + 1;
}

//citeste o singura masina, doar una
Masina citireMasinaFisier(FILE* file) {
	//functia citeste o masina dintr-un strceam deja deschis
	//masina citita este returnata;
	Masina m;
	char buffer[100];
	char delim[4] = ",;\n"; //sunt 3 pt ca e terminatorul de sir virgula si \n
	fgets(buffer, 100, file);
	char* aux;
	aux = strtok(buffer, delim);
	m.id = atoi(aux);
